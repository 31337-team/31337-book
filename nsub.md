...
- extension method ```void Returns<T>(this T value, T returnThis, params T[] returnThese)``` на результате
  ```A()```, чтобы передать значение 123 для последующих вызовов ```substitute.A()```.
...
В большинстве случаев к любому объекту framework, который важен для установки спецификации substitute или для
функционирования substitute, можно дойти через объект контекста - ```SubstitutionContext.Current``` призван
помочь в этом (шепотом: синглтон!!1111 ;) ).
...
При перехвате вызова Dynamic Proxy передает всю полезную информацию о параметрах и других свойствах через IInvocation.
