...
связи между разрозненными вызовами вспомогательных методов и перехваченных вызовов нет). Что касается act - он более
ожидаемый: выполняется запрограммированная на arrange работа через все тот же перехват методов substitute. Далее
мы постараемся углубиться в детали всех этих операций.
...
должен вернуть метод нашего substitute. Это значение, а также todo..., мы получаем через ```ICallRouter.Route()```. Итак,
